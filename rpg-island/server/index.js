...
const uri = "mongodb+srv://TTRPG:TTRPG@ttrpg.41ndrag.mongodb.net/TTRPG?retryWrites=true&w=majority"; 
...
app.put('/post', async (req, res) => {
    ...
    try {
      ...
      for (const field in formData2) {
        if (formData2[field] !== undefined && formData2[field] !== null && formData2[field] !== '') {
  ...
  app.get('/myposts', async (req, res) => {
    ...
    try {
      ...
      const userPosts = await posts.find({ user_id: userId }).sort({ data_dodania: -1 }).toArray();
...
app.post('/apply', async (req, res) => {
    ...
    try {
      ...
      if (existingApplication) {
        return res.status(409).json({ message: 'Użytkownik już zapisany do tego ogłoszenia.' });
      }
      ...
      console.error(error);
      res.status(500).json({ message: 'Wystąpił błąd podczas zapisywania użytkownika do ogłoszenia.' });
    } finally {
      await client.close();
    }
  });
...
app.get('/applicants/:postId', async (req, res) => {
  ...
  try {
    ...
    const applicants = await applicantsCollection.find({ postId: post._id.toString() }).toArray();

    // Pobierz zgłoszenia z identycznym userId z kolekcji users
    const users = [];
    for (const applicant of applicants) {
      const user = await database.collection('users').findOne({ user_id: applicant.userId });
      if (user) {
        users.push(user);
      }
    }
    ...
    res.status(500).json({ message: 'Wystąpił błąd podczas pobierania zgłoszeń użytkowników.' });
  } finally {
    await client.close();
  }
});
